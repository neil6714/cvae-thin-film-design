...
def run_optimization_pipeline(num_test_cases=10000):
    ...
    # Generate massive search space of target properties
    ...
    # Load Deep Learning infrastructure
    ...
    # 6. Extract optimal recipes
    top_candidates = df.sort_values(by='Fitness_Score', ascending=False).head(100)
    
    output_path = 'data/top_optimized_recipes.csv'
    top_candidates.to_csv(output_path, index=False)
    
    print("\n--- TOP OPTIMAL RECIPES FOUND ---")
